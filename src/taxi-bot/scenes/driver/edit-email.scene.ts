...
import { errorEditInfo, successEditEmail, WhatEmail } from '../../constatnts/message.constants';
...
@Wizard(ScenesType.EditEmailDriver)
export class EditEmailSceneDriver {
	constructor(
	...
	async onSceneEnter(@Ctx() ctx: WizardContext): Promise<string> {
		try {
			...
			this.loggerService.error('onSceneEnter Email: ' + ctx?.toString() + e?.toString());
		}
	}
	...
	async onEmail(
		...
		try {
			...
			if (valid === true) {
				...
				const { status } = await this.driverService.editEmail(chatId, msg.text);
				await ctx
					.replyWithHTML(
						successEditEmail,
